...
export async function createEvent(req: Request, res: Response) {
    ...
    const { name, description, location, startTime, endTime, capacity } = req.body;
    const { error, value } = eventSchema.validate({ name, description, location, startTime, endTime, capacity });
    ...
    try {
        ...
        if (count > 5) {
            sendResponse(res, false, 'You have reached the maximum number of open events!');
            return;
        }
        const event = await prisma.event.create({
            data: {
                ...value,
                usedId: req.body.user.id,
            },
        });
